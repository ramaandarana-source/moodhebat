...
export const VideoResultDisplay = React.forwardRef<HTMLVideoElement, VideoResultDisplayProps>(({ result, metadata, onReferenceImageChange }, ref) => {
  ...
  const handleCaptureFrame = () => {
    ...
    if (!video || video.readyState < 2) return; // Ensure video metadata is loaded
    ...
    const capture = () => {
        ...
        canvas.toBlob((blob) => {
          if (blob) {
            const file = new File([blob], 'frame-capture.png', { type: 'image/png' });
    ...
    // Set time to a fraction of a second before the end to avoid potential black frames.
