...
import { UserIcon, CopyIcon, CheckIcon, SpeakerIcon, TrashIcon } from '../icons';
...
export const ChatMessageDisplay: React.FC<{ message: ChatMessage; onDelete: (messageId: string) => void; onSelectForPreview: (url: string) => void; }> = ({ message, onDelete, onSelectForPreview }) => {
        ...
        >
            <div className={`flex items-start gap-3 max-w-full ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className="max-w-[85%] sm:max-w-xl">
                    ...
                    >
                        {message.imageUrl && (
                            <div className="mb-2 cursor-pointer" onClick={() => onSelectForPreview(message.imageUrl!)}>
                                <img src={message.imageUrl} alt="Lampiran Pengguna" className="max-w-full rounded-lg" />
                            </div>
                        )}
                        {message.role === 'model' && message.content.length === 0 && !message.imageUrl ? (
