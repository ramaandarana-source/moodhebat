...
export const Loader: React.FC<LoaderProps> = ({ message }) => {
  ...
  return (
    ...
    >
      <div className="bg-slate-900/70 border border-cyan-500/20 rounded-xl panel-glow p-4 sm:p-6 w-full">
        <div className="flex justify-between items-center border-b border-slate-700 pb-2 mb-2">
            <p className="text-sm sm:text-base text-cyan-400">{message || "Generating video..."}</p>
            <p className="text-sm sm:text-base text-slate-400">Elapsed: {formatTime(elapsedTime)}</p>
        </div>
        <div 
          ref={logContainerRef}
          className="w-full h-64 bg-black/50 rounded-md p-2 overflow-y-auto text-xs text-green-400"
        >
